...
void main() {
  test('Command APDU test', () {
    ...
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, ne: 256).toBytes(),
      "0010203000".parseHex(),
    ); //ne: 256 should be serialized as 0x00
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: null, ne: 256).toBytes(),
      "0010203000".parseHex(),
    ); //ne: 256 should be serialized as 0x00
    expect(CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, ne: 0xAABB).toBytes(), "0010203000AABB".parseHex());
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, ne: 65536).toBytes(),
      "00102030000000".parseHex(),
    ); //ne: 65536 should be serialized as 0x00 0x00
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: null, ne: 65536).toBytes(),
      "00102030000000".parseHex(),
    ); //ne: 65536 should be serialized as 0x00 0x00

    // Tets case 2
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "0102030405060708".parseHex()).toBytes(),
      "00102030080102030405060708".parseHex(),
    );
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "0102030405060708".parseHex(), ne: 0xA0).toBytes(),
      "00102030080102030405060708A0".parseHex(),
    );
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "0102030405060708".parseHex(), ne: 256).toBytes(),
      "0010203008010203040506070800".parseHex(),
    ); //ne: 256 should be serialized as 0x00
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "0102030405060708".parseHex(), ne: 0x0180).toBytes(),
      "0010203000000801020304050607080180".parseHex(),
    );
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "0102030405060708".parseHex(), ne: 65536).toBytes(),
      "0010203000000801020304050607080000".parseHex(),
    ); //ne: 65536 should be serialized as 0x00 0x00
    ...
    tv = Uint8List.fromList(
      "00102030000100".parseHex() + data + "00A0".parseHex(),
    ); // Due to extended Lc, Le is encoded as 2 bytes
    expect(CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: data, ne: 0xA0).toBytes(), tv);

    tv = Uint8List.fromList(
      "00102030000100".parseHex() + data + "0000".parseHex(),
    ); // Due to extended Lc, Le is encoded as 2 bytes
    expect(CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: data, ne: 256).toBytes(), tv);

    tv = Uint8List.fromList(
      "00102030000100".parseHex() + data + "0180".parseHex(),
    ); // Due to extended Lc, Le is encoded as 2 bytes
    ...
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30).toBytes(),
      "00102030".parseHex(),
    ); // Case 1 Command APDU
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, ne: 0x80).toBytes(),
      "0010203080".parseHex(),
    ); // Case 2s Command APDU
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, ne: 0x180).toBytes(),
      "00102030000180".parseHex(),
    ); // Case 2e Command APDU
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "41424344".parseHex()).toBytes(),
      "001020300441424344".parseHex(),
    ); // Case 3s Command APDU
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "41424344".parseHex(), ne: 0x80).toBytes(),
      "00102030044142434480".parseHex(),
    ); // Case 4s Command APDU
    expect(
      CommandAPDU(cla: 0x00, ins: 0x10, p1: 0x20, p2: 0x30, data: "41424344".parseHex(), ne: 0x180).toBytes(),
      "00102030000004414243440180".parseHex(),
    ); // Case 4e Command APDU
  }, tags: ['iso7816']);
  ...
  test('StatusWord tests', () {
    ...
    _testStatusWord(
      StatusWord.remainingAvailableResponseBytes(32),
    ...
    _testStatusWord(
      StatusWord.possibleCorruptedData,
      sw1: 0x62,
      sw2: 0x81,
      isSuccess: false,
      isWarning: true,
    ...
    _testStatusWord(
      StatusWord.selectedFileInvalidated,
      sw1: 0x62,
      sw2: 0x83,
      isSuccess: false,
      isWarning: true,
    ...
    _testStatusWord(
      StatusWord.claFunctionNotSupported,
      sw1: 0x68,
      sw2: 0x00,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.logicalChannelNotSupported,
      sw1: 0x68,
      sw2: 0x81,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.secureMessagingNotSupported,
      sw1: 0x68,
      sw2: 0x82,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.incompatibleFileStructureCommand,
      sw1: 0x69,
      sw2: 0x81,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.referencedDataInvalidated,
      sw1: 0x69,
      sw2: 0x84,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.commandNotAllowedNoEF,
      sw1: 0x69,
      sw2: 0x86,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.invalidDataFieldParameters,
      sw1: 0x6A,
      sw2: 0x80,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.notEnoughSpaceInFile,
      sw1: 0x6A,
      sw2: 0x84,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.lcInconsistentWithTLV,
      sw1: 0x6A,
      sw2: 0x85,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.lcInconsistentWithParameters,
      sw1: 0x6A,
      sw2: 0x87,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.invalidInstructionCode,
      sw1: 0x6D,
      sw2: 0x00,
      isSuccess: false,
      isWarning: false,
    ...
    _testStatusWord(
      StatusWord.noPreciseDiagnostics,
      sw1: 0x6F,
      sw2: 0x00,
      isSuccess: false,
      isWarning: false,
