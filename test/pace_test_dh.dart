...
void main() {
  test('PACE session establishment test(with DBA) - DH', () {
    ...
    Uint8List terminalPrivateKey = '5265030F751F4AD18B08AC565FC7AC952E41618D'.parseHex();
    ...
    Uint8List chipPrivateKey = '66DDAFEAC1609CB5B963BB0CB3FF8B3E047F336C'.parseHex();
    ...
    Uint8List terminalEphemeralPrivateKey = '89CCD99B0E8D3B1F11E1296DCA68EC53411CF2CA'.parseHex();
    ...
    Uint8List chipEphemeralPrivateKey = 'A5B780126B7C980E9FCEA1D4539DA1D27C342DFA'.parseHex();

    Uint8List chipEphemeralPublicKey =
    ...
    expect(
      ...
      OIE(
        identifier: [0x00, 0x04, 0x00, 0x7F, 0x00, 0x07, 0x02, 0x02, 0x04, 0x01, 0x02],
    ...
    // chip's key pair
    DHPace chip = DomainParameterSelectorDH.getDomainParameter(id: efCardAccess.paceInfo!.parameterId!);
    ...
    ResponseAPDUStep1Pace step1bChip = ResponseAPDUStep1Pace(generalAuthenticateStep1MsgChip);
    step1bChip.parse();
    ...
    Uint8List terminalGeneratorPoint = chip.getMappedGenerator(
      otherPubKey: terminal.getPubKey().toBytes(),
      nonce: nonceDecrypted,
    );
    Uint8List chipGeneratorPoint = terminal.getMappedGenerator(
      otherPubKey: chip.getPubKey().toBytes(),
      nonce: nonceDecrypted,
    );
    ...
    BigInt calcEphemeralSharedSecretTerminal = terminal.getEphemeralSharedSecret(
      otherEphemeralPubKey: chip.getPubKeyEphemeral().toBytes(),
    );

    //check shared secred
    print("Ephemeral shared secret(chip): ${Utils.bigIntToUint8List(bigInt: calcEphemeralSharedSecretChip).hex()}");
    print(
    ...
    expect(Utils.bigIntToUint8List(bigInt: calcEphemeralSharedSecretChip), sharedSecretEphemeral);

    //checking message of step 3
    Uint8List step3terminal = PACE.generateGeneralAuthenticateDataStep2and3(
      public: terminal.getPubKeyEphemeral(),
      isEphemeral: true,
    );
    ...
    Uint8List encKey = PACE.cacluateEncKey(
      ...
      seed: Utils.bigIntToUint8List(bigInt: calcEphemeralSharedSecretTerminal),
    );
