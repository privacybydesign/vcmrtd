...
void main() {
  test('TLV encoding test', () {
    ...
    expect(
      TLV.encode(0x87, "016375432908C044F6".parseHex()),
      "8709016375432908C044F6".parseHex(),
    );
    expect(
      TLV.encode(0x8E, "BF8B92D635FF24F8".parseHex()),
      "8E08BF8B92D635FF24F8".parseHex(),
    );
    expect(
      TLV.encode(0x8E, "ED6705417E96BA55".parseHex()),
      "8E08ED6705417E96BA55".parseHex(),
    );
    expect(
      TLV.encode(0x8E, "2EA28A70F3C7B535".parseHex()),
    ...
    expect(
      () => TLV.encodeLength(-1),
      throwsTLVError(
        message: "Can't encode negative or greater than 16 777 215 length",
      ),
    );
    expect(
  ...
  test('TLV decoding test', () {
    ...
    // Case 1 Select COM
    var tvRAPDU = ResponseAPDU.fromBytes(
    ...
    expect(
      () => TLV.decodeLength("82".parseHex()),
      throwsTLVError(message: "Invalid encoded length"),
    ); // Missing 2 bytes
    expect(
      () => TLV.decodeLength("8410000000".parseHex()),
      throwsTLVError(message: "Encoded length is too big"),
    ); // Encoded length too big
    expect(
      () => TLV.decode("0082".parseHex()),
      throwsTLVError(message: "Invalid encoded length"),
    ); // Missing 2 bytes
    expect(
