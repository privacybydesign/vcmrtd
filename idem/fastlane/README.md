...
Generated certificates and the provisioning profiles linked it are valid for one year.
 ...
 1. Go to the ./idem/fastlane directory
...
The three keystores the workflows expect (one per environment, see the
[workflow README](../../.github/workflows/README.md)) are generated by
`./ci_scripts/generate_keystores.sh`. Run that rather than the manual steps
below, which are kept for reference.
