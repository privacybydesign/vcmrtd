...
void main() {
  group('PACE-CAM Implementation', () {
    ...
    group('PaceInfo parameterId handling', () {
      test(
        'PaceInfo with 3 elements (parameterId present) should parse successfully',
        () {
      ...
      test(
        'PaceInfo with 2 elements (parameterId missing) should parse with parameterId=null',
        () {
          ...
          final efCardAccessNoParamId = "3111300F060a04007f00070202040202020102"
              .parseHex();
          ...
          expect(efCardAccess.paceInfo!.version, 2);
          expect(
            efCardAccess.paceInfo!.isParameterSet,
            false,
          ); // parameterId not set
      ...
      test('PaceInfo with 2 elements and different OID should also parse', () {
        ...
        //   02 01 02 - INTEGER version = 2
        final efCardAccessGm256NoParamId =
            "3111300F060a04007f00070202040204020102".parseHex();
    ...
    group('BrainpoolP256r1 (parameterId 13) support', () {
      test('Generic Mapping with BrainpoolP256r1 - test vectors from gmrtd', () {
        ...
        final sharedSecret = terminal.getSharedSecret(
        ...
        final mappedGenerator = terminal.getMappedGenerator(
          otherPubKey: chip.publicKey,
          nonce: nonce,
        );
      ...
      test('Nonce decryption with AES-128 - test vectors from gmrtd', () {
        ...
        final expectedDecryptedNonce = "3F00C4D39D153F2B2A214A078D899B22"
            .parseHex();

        final aesCipher = AESChiperSelector.getChiper(size: KEY_LENGTH.s128);
        final decryptedNonce = aesCipher.decrypt(
          data: encryptedNonce,
          key: kKdf,
        );
      ...
      test('Full PACE session keys derivation - test vectors from gmrtd', () {
        ...
        final encKey = PACE.cacluateEncKey(
          paceProtocol: efCardAccess.paceInfo!.protocol,
          seed: seed,
        );
        final macKey = PACE.cacluateMacKey(
          paceProtocol: efCardAccess.paceInfo!.protocol,
          seed: seed,
        );
    ...
    group('PACE-CAM support', () {
      ...
      test('PACE-CAM OIDs should be registered in ASN1ObjectIdentifiers', () {
        ...
        expect(
          ASN1ObjectIdentifierType.instance.hasOIDWithIdentifierString(
      ...
      test('EfCardAccess with CAM-only PaceInfo should parse successfully', () {
        ...
        final efCardAccessCamOnly =
            "31143012060a04007f00070202040604020102020110".parseHex();
      ...
      test('CardAccess with both GM and CAM entries should parse', () {
        ...
        final efCardAccessGmAndCam =
            "31283012060A04007F0007020204020202010202010D3012060A04007F0007020204060202010202010D"
                .parseHex();
      ...
      test('PACE-CAM ECAD decryption - test vectors from gmrtd', () {
        ...
        final decryptedEcadPadded = aesCipher.decrypt(
          data: ecad,
          key: ksEnc,
          iv: iv,
        );
      ...
      test('PACE-CAM session keys derivation - test vectors from gmrtd', () {
        ...
        expect(
          efCardAccess.paceInfo!.protocol.cipherAlgoritm,
          CipherAlgorithm.AES,
        );
      ...
      test('PACE-CAM mapping type should be CAM', () {
        ...
        final camOid = ASN1ObjectIdentifierType.instance
            .getOIDByIdentifierString(
              identifierString:
                  "0.4.0.127.0.7.2.2.4.6.2", // id-PACE-ECDH-CAM-AES-CBC-CMAC-128
            );
      ...
      test('PACE-CAM 192-bit and 256-bit variants should also parse', () {
        ...
        final cam192Oid = ASN1ObjectIdentifierType.instance
            .getOIDByIdentifierString(
              identifierString:
                  "0.4.0.127.0.7.2.2.4.6.3", // id-PACE-ECDH-CAM-AES-CBC-CMAC-192
            );
        ...
        final cam256Oid = ASN1ObjectIdentifierType.instance
            .getOIDByIdentifierString(
              identifierString:
                  "0.4.0.127.0.7.2.2.4.6.4", // id-PACE-ECDH-CAM-AES-CBC-CMAC-256
            );
      ...
      test('PACE-CAM terminal key generation - test vectors from gmrtd', () {
        ...
        final terminal2 = DomainParameterSelectorECDH.getDomainParameter(
          id: 13,
        );
    ...
    group('Domain parameter support flags', () {
      test('BrainpoolP256r1 (ID 13) should work despite isSupported=false', () {
        ...
        final domainParam = DomainParameterSelectorECDH.getDomainParameter(
          id: 13,
        );
      ...
      test(
        'PaceInfo.isPaceDomainParameterSupported is based on selector availability',
        () {
          ...
          final efCardAccessNist =
              "31143012060A04007F0007020204020202010202010C".parseHex();
          final efCardAccessNistParsed = EfCardAccess.fromBytes(
            efCardAccessNist,
          );

          expect(
            efCardAccessNistParsed.paceInfo!.isPaceDomainParameterSupported,
            true,
          );
    ...
    group('Full PACE flow with BrainpoolP256r1', () {
      test('Complete PACE-GM-ECDH flow - gmrtd test vectors', () {
        ...
        final terminalSharedSecret = terminal.getSharedSecret(
          otherPubKey: chip.publicKey,
        );
        final chipSharedSecret = chip.getSharedSecret(
          otherPubKey: terminal.publicKey,
        );
        expect(terminalSharedSecret, chipSharedSecret);

        // Compute mapped generator
        final mappedGen = terminal.getMappedGenerator(
          otherPubKey: chip.publicKey,
        ...
        // Ephemeral key pairs
        final terminalEphPrivKey =
            "A73FB703AC1436A18E0CFA5ABB3F7BEC7A070E7A6788486BEE230C4A22762595"
                .parseHex();
        final chipEphPrivKey =
            "107CF58696EF6155053340FD633392BA81909DF7B9706F226F32086C7AFF974A"
                .parseHex();

        terminal.setEphemeralKeyPair(
          private: terminalEphPrivKey,
          mappedGenerator: mappedGen,
        );
        chip.setEphemeralKeyPair(
          private: chipEphPrivKey,
          mappedGenerator: mappedGen,
        );

        // Compute ephemeral shared secret
        final ephSharedSecret = terminal.getEphemeralSharedSecret(
          otherEphemeralPubKey: chip.ephemeralPublicKey,
        );

        final expectedEphSharedSecret =
            "28768D20701247DAE81804C9E780EDE582A9996DB4A315020B2733197DB84925"
                .parseHex();
        expect(
          ECDHPace.ecPointToList(point: ephSharedSecret).toRelavantBytes(),
          expectedEphSharedSecret,
        );
        ...
        final seed = ECDHPace.ecPointToList(
          point: ephSharedSecret,
        ).toRelavantBytes();
        final ksEnc = PACE.cacluateEncKey(
          paceProtocol: efCardAccess.paceInfo!.protocol,
          seed: seed,
        );
        final ksMac = PACE.cacluateMacKey(
          paceProtocol: efCardAccess.paceInfo!.protocol,
          seed: seed,
        );
    ...
    group('PACE-CAM Verification', () {
      test('ECAD decryption - ICAO 9303 Part 11 Appendix I test vectors', () {
        ...
        // Decrypt ECAD
        final caIc = PaceCam.decryptEcad(
